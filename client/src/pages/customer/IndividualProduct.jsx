...
const fetchProduct = async () =>{
    await axios.get(`http://localhost:6001/fetch-product-details/${id}`).then(
        async (response)=>{
            ...
            setProductTopBid(response.data.topBidderAmount);
...
const handleBidding = async() =>{
    if(bidAmount < productTopBid || bidAmount < ProductStartPrice ){
...
const [bidInputDisplay, setBidInputDisplay] = useState(false);
    ...
    {product ?
        ...
        <div className="IndividualProduct-page">
                ...
                <div className="IndividualProduct-data">
                                ...
                                {bidInputDisplay ?
                                    ...
                                    <div className="bidding-input">
                                        <div className="form-floating mb-3">
                                            <input type='number' className="form-control" id="floatingInput6" value={bidAmount} onChange={(e)=>setBidAmount(e.target.value)} />
                                            <label htmlFor="floatingInput6">Bid amount</label>
                                        </div>
                                        <button onClick={handleBidding} >Bid now</button>
                                    </div>
                                :
                                
                                <button onClick={()=> setBidInputDisplay(true)}>Bid now</button>
                                }
                            </div>
                    ...
                    {product.status === "sold" ?
                        <>
                            <h6><b>Start Price:</b>  	&#8377; {ProductStartPrice} </h6>
                            <h6><b>Seller:</b> {productSeller} </h6>
                            <h6><b>Sold to:</b> {product.topBidderName} </h6>
            ...
            <div className="product-biddings-container">
                <h3>Biddings ({bids.length})</h3>
                <div className="product-biddings">

                    {bids.map((bid)=>(
                        <div className={product.topBidderAmount === bid.bidAmount ? "product-bidding top-bid" : "product-bidding"} key={bid._id}>
                            <span>
                                <b>Bidder Id</b>
                                <p>{bid.bidderId}</p>
                            </span>
