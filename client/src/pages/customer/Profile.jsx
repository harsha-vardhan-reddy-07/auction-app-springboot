...
const Profile = () => {
 ...
 const fetchUser = async() =>{
  ...
  const deposit = async (e)=>{
    e.preventDefault();
    await axios.post(`http://localhost:6001/deposit/${localStorage.getItem('userId')}`, {userName: localStorage.getItem('username'), transactionType: "deposit", amount: depositAmount, paymentMethod: depositMode, time: new Date()}).then(
      (response)=>{
  ...
  const withdraw = async (e)=>{
    ...
    if(withdrawAmount <= userData.funds){
        ...
        await axios.post(`http://localhost:6001/user-withdraw/${localStorage.getItem('userId')}`, {userName: localStorage.getItem('username'), transactionType:"Withdraw", amount: withdrawAmount, paymentMethod: withdrawMode, time: new Date()}).then(
          (response)=>{
...
const fetchProducts = async () =>{
  await axios.get(`http://localhost:6001/fetch-products`).then(
      (response)=>{
          const data = response.data.filter((res)=> res.topBidderId === localStorage.getItem("userId") && res.status === "sold" );
          ...
          {actionType === 'Transactions' ?
            <div className="ProfileFunds">
              ...
              {transactions.filter(transaction => transaction.userId === userId).map((transaction)=>{
                return(
                  <div className="profileTransaction" key={transaction._id} >
                    <span>
                      <h6>Amount</h6>
