...
const NewProduct = () => {
    ...
    await axios.get('http://localhost:6001/fetch-categories').then(
      (response)=>{
        if(response.data.length > 0){
          ...
          setAvailableCategories(response.data[0].categories);
      }
      }
    )
  }
  ...
  const handleNewProduct = async() =>{
    ...
    let category = productCategory === 'new category' ? productNewCategory : productCategory;

    await axios.post('http://localhost:6001/add-new-product', {sellerId: localStorage.getItem("userId"), sellerName: localStorage.getItem("username"), sellerEmail: localStorage.getItem("email"), title: productName, description: productDescription, mainImg: productMainImg, 
                      carousel: [productCarouselImg1, productCarouselImg2, productCarouselImg3], 
                       category, auctionCloseTime, startPrice: productStartPrice}).then(
      (response)=>{
        alert("product added");
        setProductName('');
